...
| hadoop / minio (S3 兼容) | 表数据落地（HDFS 或 MinIO 二选一，倾向 MinIO 简化部署） |
| kafka + zookeeper / kraft | 流式源 |
...
| deepseek-server (或 OpenAI 兼容代理) | LLM 推理后端 |
| data-agent-server | Agent Core + Tools，对外提供 CLI/HTTP 接口 |

**设计含义：**

- SandboxTool 提交目标 = 本地 docker 中的 spark-master / flink-jobmanager
- SchedulerTool 的"上线"语义 = 把代码注册到本地 docker 内的一个轻量调度（最简实现可以是 cron 容器或 Flink/Spark 自身的提交脚本），同时仍生成 PR + 工单作为代码 review 的工程证据
- Sample Baseline Service 把基线数据落到 docker 内的 HDFS/MinIO，与生产源数据**位于同一存储但不同路径**，dry-run 优先指向基线路径
...
| **SchedulerTool** | spec.deployment + 代码 + 依赖 + owners + test_cases | PR url + 工单号 + 本地 docker 部署句柄 | 写操作 / 用户确认后才调用；v1 上线目标 = **本地 Docker**，不接外部调度系统 API | 失败不重试，提示走人工流程 |
...
| 阶段 | 范围 | 验证目标 |
|---|---|---|
| **M0 环境与骨架** | docker-compose 本地栈（HMS/MinIO/Kafka/StarRocks/Spark/Flink/DeepSeek）+ Agent Core loop + Spec Accumulator + Mock MetadataTool + CodegenTool（仅 Spark SQL）+ stdout 预览 | 本地栈拉得起；端到端 loop 走通；hardcode 1-2 个无线评估典型任务 |
| **M1 真工具接通** | 真 HMS（容器内）+ ProfilerTool + ValidatorTool + SandboxTool（提交到本地 docker spark-master） | 在本地栈准备的 1-2 个真实样例数据集上完成"NL → 代码 → dry-run 预览"全流程 |
| **M2 域知识与基线** | Domain Knowledge Base（无线评估方法论字典）+ Sample Baseline Service（基线落到 MinIO/HDFS） | agent 能正确引用方法论口径；dry-run 速度可控 |
...
| **M5 闭环上线** | SchedulerTool（PR + 工单 + 本地 docker 部署句柄）+ UserPreferencesStore + Trace/Replay | 端到端从对话到 PR + 本地部署闭环 |
...
2. **本地 Docker 验证栈**可一键拉起（docker-compose），覆盖 HMS / MinIO（或 HDFS）/ Kafka / StarRocks / Spark / Flink / DeepSeek，开发机资源能承载
...
5. DeepSeek 自建部署提供 OpenAI 兼容的 chat 接口（容器内或外部均可）
