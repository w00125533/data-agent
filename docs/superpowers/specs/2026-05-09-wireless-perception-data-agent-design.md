...
| 阶段 | 范围 | 验证目标 |
|---|---|---|
| **M0 环境与骨架** | docker-compose 本地栈（HMS/HDFS/Kafka/StarRocks/Spark/Flink）+ DeepSeek API 接入 + Agent Core loop + Spec Accumulator + Mock MetadataTool + CodegenTool（仅 Spark SQL）+ stdout 预览 | 本地栈拉得起；DeepSeek API 通；端到端 loop 走通；hardcode 1-2 个无线评估典型任务 |
